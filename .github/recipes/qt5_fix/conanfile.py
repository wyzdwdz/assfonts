...
from conan.tools.microsoft.visual import vs_ide_version, _vcvars_vers, vcvars_command

def _vcvars_arch_build(conanfile):
    """
    Computes the vcvars command line architecture based on conanfile settings (host) and
    settings_build.
    """
    settings_host = conanfile.settings_build
...
class _VCVarsBuild(VCVars):
    def __init__(self, conanfile):
        super(_VCVarsBuild, self).__init__(conanfile)
    ...
    def generate(self, scope="build"):
        ...
        vcvarsarch = _vcvars_arch_build(conanfile)
        ...
        create_env_script(conanfile, content, "conanvcvars.bat", scope)
    ...
    def build(self):
        ...
        if self.settings.os == "Macos":
            ...
            if self.settings.arch == "armv8":
                if cross_building(self) and self.settings_build.arch == "x86_64":
                    args.append('QMAKE_APPLE_DEVICE_ARCHS="arm64 x86_64"')
                else:
