...
def windres_wrapper(envvars, rc):
    windres_rc = envvars.get("WINDRES_RC") if envvars.get("WINDRES_RC") else "windres-rc"
    return f"{windres_rc} {rc}"
    ...
    def generate(self):
        ...
        if is_msvc(self) or self._is_clang_cl:
            ...
            if is_msvc(self):
                ...
                rc = envvars.get("RC") if envvars.get("RC") else "rc"
                wrapped_rc = windres_wrapper(envvars, rc)
                env.define("WINDRES", wrapped_rc)
                env.define("RC", wrapped_rc)
